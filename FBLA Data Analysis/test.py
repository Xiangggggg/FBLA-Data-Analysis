...
with open('7.csv') as csv_file:
    ...
    for col in csv_reader:
        if int(col[0]) >= 1974 and int(col[0]) <= 1997:
            num = int(col[5].replace(',',''))
            x.append([int(col[0])])
            x2.append(int(col[0]))
            y.append(num)
...
ax1.set_xlabel('Year')
ax1.set_ylabel('Income-House Price Ratio',color=color)
ax1.tick_params(axis='y',labelcolor=color)

regressor = RandomForestRegressor(n_estimators = 100, random_state = 0)
regressor.fit(x, sales)

with open('westernpredictions.csv','w') as fout: 
    for i in range(2021,2030):
        fout.write(str(i)+","+str(regressor.predict([[i]])[0])+'\n')
color2 = 'tab:red'
ax2 = ax1.twinx()
...
plt.title('Income-House Price Ratio and Houses Sold by Year in Western US')
